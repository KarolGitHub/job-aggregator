...
module.exports = function filter(offer, config) {
  ...
  if (
    ...
    !config.preferredLocations.some(
      (loc) =>
        offer.location &&
  ...
  if (config.minSalary) {
    const match = String(offer.salary)
      .replace(/\s/g, '')
      .match(/(\d{2,5})/);
  ...
  if (
    ...
    offer.tech &&
    Array.isArray(offer.tech)
  ) {
    const hasTech = offer.tech.some((tech) =>
      config.allowedTech.some(
        (allowed) => tech && tech.toLowerCase().includes(allowed.toLowerCase()),
      ),
    );
  ...
  if (offer.seniority && offer.seniority.toLowerCase() === 'junior')
    return false;
